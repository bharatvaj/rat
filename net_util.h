...
int writeln(int sockfd, const char *buf, int len){
	int bwrite = write(sockfd, buf, len);
	//clog_i("CLIENT", "written buf: %s, sent: %d", buf, bwrite);
	if(bwrite > 0){
		if(bwrite < len){
		...
		if(bwrite == len){
			write(sockfd, "\n", 1);
			//clog_i("CLIENT", "write sucessful");
			return 0;
		}
	}
	else if(bwrite == 0){
		clog_i("CLIENT", "Unspecified write error");
		return -1;
	}
	else if(bwrite < 0){
		clog_i("CLIENT", "write error, exiting...");
		return -1;
	}
  return -1;
}
...
int write_file(int sockfd, const char *file){
	...
	if(fp == NULL){
		clog_e(_NET_TAG, "cannot open specified file");
		return -1;
	}
#endif
#endif
...
char *readln(int sockfd){
		...
		for(ptr=0;quit!=1;ptr++){
			...
			if(bread > 0){
				//clog_i("SERVER", "Content read[%d]: %c", ptr, buf[ptr]);
				if(buf[ptr] == '\n'){
					buf[ptr] = '\0';
					quit = 1;
				}
				...
				clog_i("SERVER", "EOF hit");
				return NULL;
			}
			else if(bread < 0){ //read error
				clog_i("SERVER", "read error exiting...");
				return NULL;
			}
		}
